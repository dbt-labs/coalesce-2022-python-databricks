def model(dbt, session):
    # get upstream data
    orders = dbt.ref("orders").toPandas()
    products = dbt.ref("stg_products").toPandas()
    order_items = dbt.ref("stg_order_items").toPandas()

    # list of unique product ids
    product_ids = sorted(list(set(products["product_id"].unique())))
